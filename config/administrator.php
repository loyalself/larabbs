...
return array(
    ...
    'menu' => [
        '用户与权限' => [
            'users',
            // 8.5 管理后台-角色权限 添加: 本章节我们将配置角色权限管理后台，分别是 roles 和 permissions
            'roles',
            'permissions',
        ],
        // 8.6 管理后台-内容 添加: 内容管理
    ...
    /**
     * 8.8 后台访问权限修改:
     * 2号用户是管理员,只拥有管理内容的权限,没有『用户管理』权限.当我们使用 2 号用户来访问后台:
     * Chrome 浏览器会报错 ERR_TOO_MANY_REDIRECTS ，意为太多跳转死循环，页面无法渲染。原因是
    administrator.php 中，我们将 home_page 选项设置为 users 页面，当我们使用 2 号用户访问 /admin 时，
    会自动跳转到 users 页面， users 页面检测到 2 号用户没有访问权限，遂重定向到后台首页 /admin 中，访问
    首页又会重定向到 users 中，所以就是死循环。解决的方法很简单，将 home_page 选项改为访问权限较低的页面如
    topics ，这个页面是所有进入后台的用户都有权限访问的.
     */
    'home_page' => 'topics',
